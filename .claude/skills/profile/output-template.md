...
**重要：最终写入 profile.json 的 JSON 结构必须严格遵循下面的字段映射。collect-new 和 collect-risk 的 Prompt 输出已使用正确的字段名，合并时直接拼装即可，不要改名或重新组织结构。**

### 字段映射

| collect-new 输出字段 | → profile.json 字段 | 说明 |
|----------|------------|------|
| profile.* | 顶层展开 | companyName, shortName, summary, industry, subIndustry, scale, mainBusiness, products, targetCustomers, businessModel, rating, tags, website, slogan, isListed, stockCode, revenueScale |
| registration.* | registration.* | **字段名 1:1 保持**：registeredName, registeredCapital, paidCapital, establishedDate, legalRepresentative, businessStatus, shareholders, branchCount(number), interpretation, organizationType, keyFindings |
...
| hiring.* | hiring.* | keySignals[] |
| competition.* | competition.* | industryRank, marketShare, mainCompetitors[], knownClients[] |
| painPoints[] | painPoints[] | {area, description, severity} |
| opportunities[] | opportunities[] | {area, description, likelihood} |
| analysisPath | analysisPath | A/B+/B |
| confidence | confidence | 20-100 |

| collect-risk 输出字段 | → profile.json 字段 | 说明 |
|----------|------------|------|
| legal.* | riskAndPolicy.legal.* | lawsuitCount, executedPerson, dishonestPerson, businessAnomalies[], equityFreeze |
| policy.isSpecializedSME | riskAndPolicy.policy.isSpecializedSME | |
| policy.smeLevel | riskAndPolicy.policy.smeLevel | |
| policy.isHighTech | riskAndPolicy.policy.isHighTech | |
| policy.digitalTransformation.hit | riskAndPolicy.policy.digitalTransformation | 布尔值 |
| policy.esgGreen.hit | riskAndPolicy.policy.esgGreen | 布尔值 |
| 命中的政策标签 | tags[]（追加） | 如"专精特新""高新技术"等追加到 tags |
